...
当前增量：**v0.24 领地与军团AI**。领地着色、前线定位、快捷托管与接敌修复见[说明](docs/TERRITORY_AI_V0_24.md)，验收状态见[进度](progress.md)。

独立运行的 Android 三国回合制策略游戏工程，最终目标参照 **PC《三国志11＋威力加强版》** 的完整系统范围。横竖屏、中文、离线游玩；不依赖模拟器、原版可执行文件或远程串流。

## 当前状态

v0.24 已实现领地着色、前线定位、军团快捷托管和接敌修复，安装验收进行中。详见[本轮规则与边界](docs/TERRITORY_AI_V0_24.md)。
...
5. 「菜单 → 保存局面 / 读取存档」支持3个手动槽。自动存档在成功操作和进入后台时更新；当前写入v18，能读取v1～v17旧存档，旧手动槽对应现在的槽1。
