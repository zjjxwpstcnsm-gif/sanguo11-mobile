...
## v0.52.0 · 快速旬结算、天下总览与战法高光

完整接续 v0.51 `2ff086d25fff`，保留18项地图/美术指纹。人员城际路线及关系查询缓存保留六旬规则/RNG逐字节结果；逐势力规则仍顺序执行，动画不再阻塞后续计算。默认8秒演示预算，10秒为250剧本28势力完整过场的验收目标，不以截断AI实现；主动暂停/完整演示不受此时限约束。

新增全国势力、部队、港口、关卡、设施总览，联动现有武将/城池列表；势力详情提供资产、定期收支预测、技巧树及PK能力研究。行动力常驻左上；开局真实地图着色点选势力；实际战法暴击显示680ms武将高光，复用现有头像而非新增原版大立绘。

[实现、统计口径与边界](docs/REALM_TURN_V0_52.md) · [验证与发布身份](docs/validation/v052/README.md)。Android实测结果、截图和准确构建提交以对应Actions制品为准，不把本地纯计算当真机完整过场。
...
以用户提供的 7200×6752 参考图配准42城，校正40座港关，六区分块细化3,535格；九剧本同步，保留591处开发容量和原有全部美术。35港逐一检查实际上下船通道；10关验证敌我通行，防止六角格斜穿关隘。旧存档仍保留原地图，需新开局查看新地理。

[地图范围、来源与边界](docs/MAP_FIDELITY_V0_50.md) · [验证记录](docs/validation/v050/README.md)。未逐格复核的东北/南中陆地及不能从图中辨别的湿地/浅滩细类仍保留旧配置，不宣称全图100%完成。
...
当前版本：**v0.52.0 · 快速旬结算与天下总览**。保留 v0.49 部队、忠诚与粮秣、v0.48逐势力播放，以及v0.46墨玉界面。

保留 v0.45.0 地理与连续目标任务：西北绿洲、林地与城际道路；许昌—新野森林带；曲阿港与壶关在v0.50按原图再次校准，详见该轮记录。下河必须经己方港口；地图选择目标会先验证可达性，再自动行军并逐旬攻击、进驻或修理。沿用v44美术、沙地规则、黄河、着色和生产产能。
