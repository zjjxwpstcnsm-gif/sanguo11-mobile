...
final class CustomOfficerProbe {
    ...
    private void runChecks(String mode)throws Exception{
        ...
        World commands=CustomOfficerSetup.apply(context(),fixture,testRoot);commands.city(10).order=50;ui(()->{call(game,"activateWorld",new Class<?>[]{World.class},commands);World.Result patrol=SessionProbe.command(game,draft->draft.patrol(10,firstId));check(patrol.ok,"installed custom officer completes domestic patrol");});
        World commandsAfterPatrol=SaveCodec.decode(SaveCodec.encode(SessionProbe.view(game)));World.Result turn=commandsAfterPatrol.nextTurn();check(turn.ok,"real turn ends to restore action allowance");World.Result deploy=commandsAfterPatrol.army.deploy(10,firstId,new int[]{secondId},World.Weapon.SPEAR,Army.Ship.BOAT,3000,6000);check(deploy.ok,"installed real sortie with custom commander and deputy: "+deploy.message);World.Unit unit=commandsAfterPatrol.units.stream().filter(u->u.officerId==firstId).findFirst().orElseThrow();check(commandsAfterPatrol.war.plotCost(unit.id,War.Plot.CONFUSE)==1,"installed custom skill affects official cost");
        // Legal battlefield fixture; attack remains the normal public command, not a test formula.
        World.Officer enemyOfficer=new World.Officer(990001,"测试守军",1,-1,70,70,70,70,70);commandsAfterPatrol.officers.add(enemyOfficer);World.Unit enemy=new World.Unit(commandsAfterPatrol.nextUnitId++,1,enemyOfficer.id,World.Weapon.SPEAR,new Hex(13,12),5000,10000);enemyOfficer.cityId=-1;enemyOfficer.unitId=enemy.id;commandsAfterPatrol.units.add(enemy);unit.hex=new Hex(12,12);unit.acted=false;int enemyTroops=enemy.troops;ui(()->{SessionProbe.install(game,commandsAfterPatrol);game.selectUnitAndFocus(unit.id);World.Result hit=SessionProbe.command(game,draft->draft.attack(unit.id,enemy.id));check(hit.ok&&SessionProbe.view(game).unit(enemy.id).troops<enemyTroops,"installed real attack inflicts damage");});settle();shot("08-custom-crew-battle");
        byte[] save=SaveCodec.encode(SessionProbe.view(game));try(OutputStream out=context().openFileOutput("auto.sg11",0)){out.write(save);}check(CustomOfficers.portrait(commandsAfterPatrol,firstId).png.length>0,"campaign embeds portrait bytes");
