...
public final class GameSmokeRunner extends Instrumentation {
    private String customOfficersMode="";
    private Activity current;
    private String displacement="";
    private String recovery="";
    private boolean governance66,road62,road62Baseline,reference61,reference60,reference59,reference58,mapTap57,repro57,native56,city55,reports53,realm52,map51,map50,systems49,flow48,replay47,polish46,map42,balance41,architecture33,navigation32,mapPerformance,fidelity,upgradeOnly,upgrade25,upgrade26,upgrade27,experience,armyOnly;
    @Override public void callActivityOnResume(Activity a){super.callActivityOnResume(a);current=a;}
    @Override public void onCreate(Bundle arguments){super.onCreate(arguments);customOfficersMode=arguments==null?"":arguments.getString("customOfficers","");governance66=arguments!=null&&"true".equals(arguments.getString("governance66"));road62=arguments!=null&&"true".equals(arguments.getString("road62"));road62Baseline=arguments!=null&&"true".equals(arguments.getString("road62Baseline"));reference61=arguments!=null&&"true".equals(arguments.getString("reference61"));reference60=arguments!=null&&"true".equals(arguments.getString("reference60"));reference59=arguments!=null&&"true".equals(arguments.getString("reference59"));reference58=arguments!=null&&"true".equals(arguments.getString("reference58"));mapTap57=arguments!=null&&"true".equals(arguments.getString("mapTap57"));repro57=arguments!=null&&"true".equals(arguments.getString("repro57"));native56=arguments!=null&&"true".equals(arguments.getString("native56"));city55=arguments!=null&&"true".equals(arguments.getString("city55"));reports53=arguments!=null&&"true".equals(arguments.getString("reports53"));realm52=arguments!=null&&"true".equals(arguments.getString("realm52"));map51=arguments!=null&&"true".equals(arguments.getString("map51"));map50=arguments!=null&&"true".equals(arguments.getString("map50"));systems49=arguments!=null&&"true".equals(arguments.getString("systems49"));flow48=arguments!=null&&"true".equals(arguments.getString("flow48"));replay47=arguments!=null&&"true".equals(arguments.getString("replay47"));polish46=arguments!=null&&"true".equals(arguments.getString("polish46"));map42=arguments!=null&&"true".equals(arguments.getString("map42"));balance41=arguments!=null&&"true".equals(arguments.getString("balance41"));armyOnly=arguments!=null&&"true".equals(arguments.getString("army"));architecture33=arguments!=null&&"true".equals(arguments.getString("architecture33"));navigation32=arguments!=null&&"true".equals(arguments.getString("navigation32"));mapPerformance=arguments!=null&&"true".equals(arguments.getString("mapPerformance"));fidelity=arguments!=null&&"true".equals(arguments.getString("fidelity"));displacement=arguments==null?"":arguments.getString("displacement","");recovery=arguments==null?"":arguments.getString("recovery","");upgrade27=arguments!=null&&"27".equals(arguments.getString("upgrade"));experience=arguments!=null&&"true".equals(arguments.getString("experience"));upgradeOnly=arguments!=null&&"true".equals(arguments.getString("upgrade"));upgrade25=arguments!=null&&"25".equals(arguments.getString("upgrade"));upgrade26=arguments!=null&&"26".equals(arguments.getString("upgrade"));start();}
    @Override public void onStart(){
        Bundle result=new Bundle();
        try {
            if(!customOfficersMode.isEmpty()){new CustomOfficerProbe(this).run(customOfficersMode);result.putString("stream","CUSTOM OFFICERS ANDROID PASS: "+customOfficersMode+"\n");finish(Activity.RESULT_OK,result);return;}
            if(governance66){new Governance66Probe(this).run();result.putString("stream","GOVERNANCE66 ANDROID PASS: complete date, ruler labels, faction/officer tables, batch reward, emperor nation and auto arrival.\n");finish(Activity.RESULT_OK,result);return;}
            if(road62||road62Baseline){new Road62Probe(this,road62).run();result.putString("stream",road62?"ROAD62 ANDROID PASS: installed ordinary-road pixel equivalence and retained identity.\n":"ROAD62 BASELINE PASS: exact old APK has distinct road pixels and preserved saves.\n");finish(Activity.RESULT_OK,result);return;}
            if(reference61){new Reference61Probe(this).run();result.putString("stream","REFERENCE61 ANDROID PASS: full-area paired corners, explicit scenery, unknown/padding exclusions, exact old58/59/60 saves, source labels, opening previews, minimap, legal march and retained map gameplay.\n");finish(Activity.RESULT_OK,result);return;}
            if(reference60){new Reference60Probe(this).run();result.putString("stream","REFERENCE60 ANDROID PASS: new water batches, true old58/59 saves, normal new-game controls, real MapView touch/LOD/crops/minimap, labelled structures and legal/rejected regional march.\n");finish(Activity.RESULT_OK,result);return;}
            if(reference59){new Reference59Probe(this).run();result.putString("stream","REFERENCE59 ANDROID PASS: regional corrections, actual old58 saves, real map touches, Q rules, crops, LOD, minimap, labelled walls and regional march.\n");finish(Activity.RESULT_OK,result);return;}
            if(reference58){new Reference58Probe(this).run();result.putString("stream","REFERENCE58 ANDROID PASS: real revised terrain touches, shared minimap projection, crops, LOD, labelled walls, raw old saves and corrected-region march.\n");finish(Activity.RESULT_OK,result);return;}
            if(repro57){new RoadCrash57Probe(this).run();throw new AssertionError("Baseline ROAD must crash, not silently pass");}
            if(mapTap57){new MapTap57Probe(this).run();result.putString("stream","MAP57 ANDROID PASS: actual ROAD/DAM/wall taps, source voids, crops, pan/zoom, LOD, neutral entities and old-save preservation.\n");finish(Activity.RESULT_OK,result);return;}
            if(native56){new Native56Probe(this).run();result.putString("stream","NATIVE56 ANDROID PASS: installed map, city LODs, seven-cell touch, real sortie/transit/entry/combat, next turn, minimap and saves.\n");finish(Activity.RESULT_OK,result);return;}
            if(city55){new City55Probe(this).run();result.putString("stream","CITY55 ANDROID PASS: actual seven-cell selection, sortie, paid city transit, next turn, explicit garrison, edge attack and saves.\n");finish(Activity.RESULT_OK,result);return;}
            if(reports53){new Reports53Probe(this).run();result.putString("stream","REPORTS53 PASS: installed battle reports, single-list crew, tab state, rotation and actual sortie.\n");finish(Activity.RESULT_OK,result);return;}
            if(realm52){new Realm52Probe(this).run();result.putString("stream","REALM52 PASS: real 250/28 timing, exact outcomes, all lists, AP, map faction selection and critical cut-ins.\n");finish(Activity.RESULT_OK,result);return;}
            if(map51){new Map51Probe(this).run();result.putString("stream","MAP51 PASS: shipped northeast, southern regions, original artwork and immutable world.\n");finish(Activity.RESULT_OK,result);return;}
            if(map50){new Map50Probe(this).run();result.putString("stream","MAP50 PASS: official geography, regional rendering, orientation and immutable world.\n");finish(Activity.RESULT_OK,result);return;}
            if(systems49){new Personnel49Probe(this).run();result.putString("stream","SYSTEMS49 PASS: unit stats, adviser and real deployment slider/rotation/stock checks.\n");finish(Activity.RESULT_OK,result);return;}
            if(flow48){new Turn48Probe(this).run();result.putString("stream","FLOW48 PASS: faction streaming, rotation/skip, facility actions, inline deployment and fresh drafts.\n");finish(Activity.RESULT_OK,result);return;}
            if(replay47){new Turn47Probe(this).run();result.putString("stream","REPLAY47 PASS: movement, attack/hit, plot, pause, rotation, controls, skip and authoritative save.\n");finish(Activity.RESULT_OK,result);return;}
            if(polish46){new Polish46Probe(this).run();result.putString("stream","POLISH46 PASS: overlay geometry, cached refresh, instant tap, native tables, motion interruption and portrait/landscape screenshots.\n");finish(Activity.RESULT_OK,result);return;}
            ...
            if(mapPerformance){
                World national=TestScenarios.load("heroes-mobile-sandbox",1);
                try(FileOutputStream out=getTargetContext().openFileOutput("auto.sg11",0)){out.write(SaveCodec.encode(national));}
                startActivitySync(new Intent(getTargetContext(),MainActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));waitForIdleSync();
    ...
    private void fidelityFlow()throws Exception {
        ...
        World national=TestScenarios.load("heroes-mobile-sandbox",1);World.City city=national.city(20017);
    ...
    private void tacticalDisplacementFlow()throws Exception {
        ...
        installFixture(TestScenarios.load("regional-sandbox",2),new Hex(1,1));
    }
    ...
    private void experienceFlow()throws Exception {
        World initial=TestScenarios.load("regional-sandbox",2);
        try(FileOutputStream out=getTargetContext().openFileOutput("auto.sg11",0)){out.write(SaveCodec.encode(initial));}
        startActivitySync(new Intent(getTargetContext(),MainActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));waitForIdleSync();
        try(PrintWriter out=new PrintWriter(new File(getTargetContext().getExternalFilesDir(null),"experience.txt"))){
            ...
            for(String orientation:new String[]{"竖屏","横屏"}){
                ...
                World nation=TestScenarios.load("heroes-mobile-sandbox",0);installFixture(nation,nation.home().hex);territoryMode("势力范围 · 同势力合并");click("全图",true);out.println(orientation+" full-map scale="+camera(mapView()).scale+" min="+camera(mapView()).minScale);require(Math.abs(camera(mapView()).scale-camera(mapView()).minScale)<.001f,"explicit full map reaches national scale");screenshot("v028-national-"+orientation);
                territoryMode("关闭领地着色");
            }
            World nation=TestScenarios.load("heroes-mobile-sandbox",0);installFixture(nation,nation.home().hex);benchmarkExperience(out,nation,"42cities-670officers");
    ...
    private void experienceRegressions()throws Exception {
        ...
        for(String orientation:new String[]{"竖屏","横屏"}){
            ...
            installFixture(TestScenarios.load("regional-sandbox",2),new Hex(18,10));locateCity("建业");click("军事",true);click("快速出征（单将）",true);click("甘宁 ·",false);click("弩兵 ·",false);setInput("兵力数量","2345");
    ...
    private void nationalReturnPosition()throws Exception {
        World nation=TestScenarios.load("heroes-mobile-sandbox",0);installFixture(nation,nation.home().hex);chooseOrientation("竖屏");allCityStates();clickNav("城市");click("展开",true);
    ...
    private void strategicManagementFlow()throws Exception {
        byte[] original=SaveCodec.encode(saved());World w=TestScenarios.load("world-drill",0);w.city(11).food=0;installFixture(w,w.city(11).hex);
        for(String orientation:new String[]{"竖屏","横屏"}){
    ...
    private void territoryAiFlow()throws Exception {
        byte[] original=SaveCodec.encode(saved());
        World nation=TestScenarios.load("heroes-mobile-sandbox",0);installFixture(nation,nation.home().hex);
        byte[] before=SaveCodec.encode(saved());
        for(String orientation:new String[]{"竖屏","横屏"}){
        ...
        World w=TestScenarios.load("world-drill",0);int mainAp=w.actionPoints[0];installFixture(w,w.city(11).hex);locateCity("北境城");
    ...
    private void mobileShortcutsFlow()throws Exception {
        byte[] original=SaveCodec.encode(saved());
        for(String orientation:new String[]{"竖屏","横屏"}){
            World w=TestScenarios.load("regional-sandbox",2);installFixture(w,w.city(310).hex);chooseOrientation(orientation);
        ...
        World heroes=TestScenarios.load("heroes-mobile-sandbox",0);installFixture(heroes,heroes.home().hex);click("全图",true);screenshot("v023-42-city-map");
    ...
    private void lifecycleFlow()throws Exception {
        World w=TestScenarios.load("lifecycle-drill",0);w.turn=2;installFixture(w,w.city(10).hex);
        byte[] before=SaveCodec.encode(saved());
        ...
        try(InputStream in=ScenarioCatalog.class.getResourceAsStream("/test-scenarios/lifecycle-drill.properties");OutputStream out=new FileOutputStream(source)){byte[] b=new byte[4096];int n;while((n=in.read(b))!=-1)out.write(b,0,n);}
    ...
    private void strategicFlow()throws Exception {
        installFixture(TestScenarios.load("regional-sandbox",2),new Hex(18,10));waitForIdleSync();
    ...
    private void mobileFlow() throws Exception {
        ...
        installFixture(TestScenarios.load("regional-sandbox",2),new Hex(18,10));
    ...
    private void logisticsFlow()throws Exception {
        ...
        allCityStates();installFixture(TestScenarios.load("regional-sandbox",2),new Hex(18,10));
    }
    ...
    private void contestFlow()throws Exception {
        startTestScenario("contest-drill",0);
        ...
        for(int seed=0;seed<100&&pending==null;seed++){
            World candidate=TestScenarios.load("contest-drill",0);candidate.strategy.setSeed(seed);candidate.contests.persuade(10,2,6);
    ...
    private void worldFlow()throws Exception {
        startTestScenario("world-drill",0);
        ...
        w=TestScenarios.load("world-drill",0,41);for(int seed=0;seed<100;seed++){w.strategy.setSeed(seed);World trial=SaveCodec.decode(SaveCodec.encode(w));trial.war.plot(1,trial.unit(4).hex,War.Plot.LIGHTNING);if(trial.unit(4).troops<w.unit(4).troops)break;}
        ...
        w=TestScenarios.load("world-drill",0,41);for(int seed=0;seed<100;seed++){w.strategy.setSeed(seed);World trial=SaveCodec.decode(SaveCodec.encode(w));trial.campaign.negotiate(10,1,1,Campaign.TreatyKind.CEASEFIRE,6);if(trial.contests.busy())break;}
    ...
    private void estatesFlow()throws Exception {
        startTestScenario("estates-drill",0);waitForIdleSync();
    ...
    private void fieldworkFlow()throws Exception {
        World w=TestScenarios.load("fieldworks-drill",0);w.city(20).troops=0;w.city(21).troops=0;
    ...
    private void armyFlow()throws Exception {
        startTestScenario("river-siege-sandbox",0);
        ...
        World factory=TestScenarios.load("river-siege-sandbox",0);for(World.City city:factory.cities)if(city.owner==1){city.gold=0;city.troops=0;}
    ...
    private void rulesFlow()throws Exception {
        World w=TestScenarios.load("river-siege-sandbox",0);
    ...
    private void sourceProfileFlow()throws Exception {
        World w=TestScenarios.load("regional-sandbox",0);installFixture(w,w.city(100).hex);
    ...
    private void contentFlow()throws Exception {
        startTestScenario("officer-reference-drill",2);screenshot("28-sourced-opening");waitForIdleSync();
        ...
        float[] old={0,0};runOnMainSync(()->{old[0]=camera.centerX();old[1]=camera.centerY();});int[] pos=new int[2];float[] target=new float[2];runOnMainSync(()->{map.getLocationOnScreen(pos);float den=map.getResources().getDisplayMetrics().density;target[0]=pos[0]+map.getWidth()-20*den;target[1]=pos[1]+72*den;});
