...
final class Map51Probe {
    ...
    Map51Probe(Instrumentation test){this.test=test;}
    void run() throws Exception {
        ...
        Hex restricted=MapCoordinates.axial(69,7,100);
        require(world.terrain[restricted.q][restricted.r]==World.Terrain.MOUNTAIN_PATH,"source red pass is bundled as technology-gated terrain");
        Hex poison=MapCoordinates.axial(11,84,100);
        require(world.terrain[poison.q][poison.r]==World.Terrain.POISON,"Jianning poison is present in installed official scenario");
        byte[] before=SaveCodec.encode(world);
        test.runOnMainSync(()->{page();map.fit();});settle();shot("01-national");
        int[][] centers={{61,8},{74,10},{86,8},{66,22},{80,25},{13,82},{3,93},{45,91}};
        String[] names={"02-ji","03-beiping","04-xiangping","05-nanpi-pingyuan","06-beihai","07-jianning","08-yunnan","09-jingnan"};
        ...
        try(Writer out=new OutputStreamWriter(new FileOutputStream(new File(dir,"v051-checks.txt")),"UTF-8")){out.write("PASS: "+checks+" installed map checks\n"+report);}
    }
    ...
    private void shot(String name)throws Exception{Bitmap image=test.getUiAutomation().takeScreenshot();require(image!=null,"screenshot "+name);File dir=test.getTargetContext().getExternalFilesDir("smoke");if(dir==null)throw new IOException("Missing evidence directory");dir.mkdirs();try(OutputStream out=new FileOutputStream(new File(dir,"v051-"+name+".png"))){image.compress(Bitmap.CompressFormat.PNG,100,out);}image.recycle();}
