...
final class BuildPicker {
    ...
    void show(){
        ...
        TextView description=a.text(c.name+" · 开发地 "+h.q+","+h.r+"\n已用 "+w.domestic.count(c.id)+" / "+w.development.capacity(c.id)+" · 可用金 "+c.gold,14,a.paper);form.addView(description);
        ...
        ListView list=new ListView(a);list.setContentDescription("建设选项列表");form.addView(list,new LinearLayout.LayoutParams(-1,a.dp(220)));
        ...
        Runnable refresh=()->{
            ...
            count.setText(filtered.isEmpty()?"没有可用选项":"共 "+filtered.size()+" 项"+(officers?" · 按政治排序":" · 建成即最高等级"));page.setText((current[0]+1)+" / "+pages);previous.setEnabled(current[0]>0);next.setEnabled(current[0]+1<pages);draft.putInt("page",current[0]);remember();
        };
        AlertDialog dialog=new AlertDialog.Builder(a).setTitle(officers?"建设 · 选择执行人":"开发地 · 选择设施").setView(form).setNegativeButton("取消",(d,n)->a.closeForm()).setNeutralButton(officers?"上一步":"返回地图",(d,n)->{if(officers){draft.remove("facility");draft.putInt("page",0);show();}else a.closeForm();}).create();
        dialog.setOnCancelListener(d->a.closeForm());
        list.setOnItemClickListener((parent,v,i,id)->{
            Object selected=visible.get(i);dialog.dismiss();
