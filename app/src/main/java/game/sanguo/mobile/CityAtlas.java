...
/** a063 generated city/gate/port art. All LODs retain buildings; missing art is an error,
...
final class CityAtlas {
    ...
    static synchronized void load(Context context){
        ...
        Map<String,Sprite[]> pending=new HashMap<>();Map<String,Bitmap> decoded=new HashMap<>();long bytes=0;
        try(InputStream input=context.getAssets().open("map/cities-a063/catalog.json")){
            JSONObject root=new JSONObject(new String(read(input),StandardCharsets.UTF_8));
            if(root.getInt("asset_revision")!=CityArtCatalog.ASSET_REVISION)throw new IOException("City atlas asset revision mismatch");
            ...
            for(Iterator<String> keys=variants.keys();keys.hasNext();){String key=keys.next();JSONObject lods=variants.getJSONObject(key);Sprite[] sprites=new Sprite[3];
                ...
                for(int i=0;i<names.length;i++){JSONObject entry=lods.getJSONObject(names[i]);
                    try(InputStream png=context.getAssets().open("map/cities-a063/"+entry.getString("file"))){
                        BitmapFactory.Options opts=new BitmapFactory.Options();opts.inScaled=false;opts.inPreferredConfig=Bitmap.Config.ARGB_8888;
                        Bitmap bitmap=decoded.get(entry.getString("file"));
                        if(bitmap==null){bitmap=BitmapFactory.decodeStream(png,null,opts);if(bitmap==null)throw new IOException("Cannot decode city "+key+"/"+names[i]);decoded.put(entry.getString("file"),bitmap);bytes+=bitmap.getAllocationByteCount();}
                        sprites[i]=new Sprite(bitmap,entry);
                    }
            ...
            for(String key:new String[]{"gate","port","gate-ns","gate-ew","gate-nesw","gate-nwse","port-n","port-s","port-e","port-w","port-ne","port-nw","port-se","port-sw"})
                if(!pending.containsKey(key))throw new IOException("Missing directional site art "+key);
        }catch(IOException|JSONException error){for(Bitmap bitmap:decoded.values())bitmap.recycle();throw new IllegalStateException("a063 城池资源缺失，请重新安装完整 APK",error);}
        CATALOG.putAll(pending);decodedBytes=bytes;
    }
    static String mapKey(World world,World.City site){return CityArtCatalog.visualKey(world,site);}
    ...
    static void drawMap(Canvas c,World world,World.City city,float x,float y,float scale,int owner){
        String key=mapKey(world,city);Sprite s=sprite(key,100);draw(c,key,x,y,s.width,scale,owner);
    }
