...
final class ContestUi {
    ...
    ContestUi(Activity a,World w,Consumer<World.Result> apply){this.a=a;this.w=w;this.apply=apply;}
    ...
    void challenge(World.Unit actor){
        ...
        new AlertDialog.Builder(a).setTitle("选择单挑目标").setItems(labels,(d,n)->{
            World.Unit target=targets.get(n);confirm("发起单挑","消耗10气力和本旬行动；对方可能拒绝。\n当前上阵武将体力归零即败，五十合平手。\n主将败北可能被俘并导致部队解散，副将败北则仅退出编队。",()->apply.accept(w.contests.challenge(actor.id,target.id)));
    ...
    private void duel(LinearLayout panel,Contests.Session s){
        ...
        for(Duel.Move move:Duel.Move.values())if(move!=Duel.Move.SWAP){
            ...
            button(panel,move.label+(move.cost>0?" · 斗志"+move.cost:""),error==null,()->apply.accept(w.contests.duelMove(id,revision,Duel.Stance.values()[stance.getSelectedItemPosition()],move,-1)));
        }
        for(int i=0;i<d.fighters(0).size();i++){
            final int index=i;Duel.Fighter f=d.fighters(0).get(i);if(f==d.active(0))continue;
            button(panel,"换将 · "+w.officer(f.officerId()).name+(f.joined()?" · 体力"+f.hp():" · 等待支援"),d.moveError(w,Duel.Move.SWAP,index)==null,()->apply.accept(w.contests.duelMove(id,revision,Duel.Stance.values()[stance.getSelectedItemPosition()],Duel.Move.SWAP,index)));
        }
        button(panel,"认输并结束单挑",true,()->confirm("认输","当前上阵武将直接判负，按败北处置；这不是退却。",()->apply.accept(w.contests.concede(id,revision))));
