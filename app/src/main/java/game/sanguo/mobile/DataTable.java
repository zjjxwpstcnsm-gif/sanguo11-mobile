...
final class DataTable<T> extends LinearLayout {
    ...
    DataTable(Activity a,List<T> options,List<Column<T>> columns,int[] visible,Function<T,String> summary,ToLongFunction<T> key,Consumer<T> select,Consumer<T> detail){
        super(a);this.a=a;this.columns=columns;this.visible=visible.clone();this.summary=summary;this.key=key;source.addAll(options);setOrientation(VERTICAL);indexSearch();
        searchBar=new LinearLayout(a);searchBar.setGravity(Gravity.CENTER_VERTICAL);searchBar.setPadding(dp(2),dp(4),dp(2),dp(4));
        ...
        Button clear=CompactButtons.create(a);clear.setText("×");clear.setContentDescription("清空搜索");clear.setOnClickListener(v->search.setText(""));searchBar.addView(clear,new LayoutParams(dp(44),dp(48)));addView(searchBar);
        groups=new LinearLayout(a);groups.setVisibility(GONE);addView(groups,new LayoutParams(-1,dp(44)));
        count=new TextView(a);UiTheme.text(count);count.setTextColor(UiTheme.MUTED);count.setTextSize(11);count.setPadding(dp(6),dp(4),dp(6),dp(4));count.setAccessibilityLiveRegion(ACCESSIBILITY_LIVE_REGION_POLITE);addView(count);
        ...
        header=new LinearLayout(a);header.setBackgroundColor(0xff263b44);grid.addView(header,new LayoutParams(-1,dp(44)));
        ...
        adapter=new BaseAdapter(){
            ...
            public View getView(int i,View reuse,ViewGroup parent){
                ...
                T item=getItem(i);row.setMinimumHeight(dp(46));row.setBackgroundColor(selected.test(item)?0xff2b514a:i%2==0?0xff152630:0xff1b2e38);
    ...
    void columnGroups(String[] labels,int[][] choices){
        ...
        for(int i=0;i<labels.length;i++){final int index=i;Button b=CompactButtons.create(a);b.setText(labels[i]);b.setSelected(i==group);b.setContentDescription("表格列组 · "+labels[i]);b.setOnClickListener(v->{group=index;visible=choices[index].clone();for(int n=0;n<groups.getChildCount();n++)groups.getChildAt(n).setSelected(n==group);rebuildColumns(contentWidth);refresh();});groups.addView(b,new LayoutParams(0,-1,1));}
    ...
    void refresh(){removeCallbacks(filterTask);shown.clear();String q=search.getText().toString().trim().toLowerCase(Locale.ROOT);
        ...
        updateHeaders();count.setText(shown.isEmpty()?"没有符合条件的对象 · 可清空搜索重试":"共 "+shown.size()+" 项 · 点击列名排序 · 长按详情");adapter.notifyDataSetChanged();
    }
