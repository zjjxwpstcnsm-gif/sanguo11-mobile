...
final class OverviewUi {
    ...
    View officers() {
        ...
        EditText search=new EditText(a);search.setSingleLine();search.setTextColor(a.paper);search.setHintTextColor(a.muted);search.setHint("搜索武将姓名");search.setContentDescription("搜索武将姓名");search.setText(state.query);host.addView(search,new LinearLayout.LayoutParams(-1,a.dp(48)));
        LinearLayout filters=new LinearLayout(a);host.addView(filters);
        Button faction=a.button(state.owner<0?"全部势力":w.faction(state.owner),v->{String[] labels=new String[w.factions.length+1];labels[0]="全部势力";System.arraycopy(w.factions,0,labels,1,w.factions.length);new AlertDialog.Builder(a).setTitle("按势力筛选").setItems(labels,(d,i)->{state.owner=i-1;a.refresh();}).show();});
        ...
        Rows<World.Officer> adapter=list(host,UiModels.officers(w,state.query,state.owner,state.city,state.officerSort),o->o.id,o->o.name+" · "+w.faction(o.owner),
