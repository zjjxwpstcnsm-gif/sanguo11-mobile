...
final class OverviewUi {
    ...
    private <T> Rows<T> list(LinearLayout host,List<T> rows,ToLongFunction<T> key,Function<T,String> title,Function<T,String> detail,Consumer<T> select,String empty) {
        ...
        Rows<T> adapter=new Rows<>(rows,key,title,detail);adapter.emptyView=blank;list.setAdapter(adapter);if(state.page.equals("cities"))list.setSelectionFromTop(state.cityPosition,state.cityTop);list.setOnItemClickListener((p,v,index,id)->select.accept(adapter.getItem(index)));return adapter;
    }
    ...
    View cities() {
        ...
        controls.addView(a.button(state.cityOwner==-1?"全部城池":state.cityOwner==-2?"中立城池":w.faction(state.cityOwner),v->new AlertDialog.Builder(a).setTitle("所属势力").setItems(owners,(dialog,i)->{state.cityOwner=i==0?-1:i==1?-2:i-2;state.cityPosition=0;a.refresh();}).setNegativeButton("取消",null).show()),new LinearLayout.LayoutParams(0,a.dp(48),1));
