...
final class RealmUi {
    ...
    private void nameNation(int side){
        ...
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v->{if(!a.currentWorld(w))return;World.Result r=a.apply(w,()->w.governance.nameNation(side,field.getText().toString()));if(!r.ok){field.setError(r.message);return;}dialog.dismiss();});
    }
