...
final class SceneCamera {
    float x,z,span=35,tilt=55; int width=1,height=1;
    ...
    float screenX(float wx){return width*.5f+(wx-x)*pixels();}
    float screenY(float wz,float y){return (float)(height*.5f+((wz-z)*sin()-y*cos())*pixels());}
    float worldX(float sx){return x+(sx-width*.5f)/pixels();}
    float worldZ(float sy){return (float)(z+(sy-height*.5f)/pixels()/sin());}
    void pan(float dx,float dy){x-=dx/pixels();z-=dy/pixels()/sin();}
