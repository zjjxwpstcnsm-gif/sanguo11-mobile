...
final class ScenePicking {
    static float hit(SceneCamera camera,SceneMesh mesh,float x,float y,float z,float yaw,float scale,float sx,float sy){
        ...
        for(int i=0;i<ids.length;i+=3){
            ...
            float awx=x+c*v[a]+s*v[a+2],awz=z-s*v[a]+c*v[a+2];
            float ax=camera.screenX(awx,awz),ay=camera.screenY(awx,awz,y+scale*v[a+1]);
            float bwx=x+c*v[b]+s*v[b+2],bwz=z-s*v[b]+c*v[b+2];
            float bx=camera.screenX(bwx,bwz),by=camera.screenY(bwx,bwz,y+scale*v[b+1]);
            float dwx=x+c*v[d]+s*v[d+2],dwz=z-s*v[d]+c*v[d+2];
            float dx=camera.screenX(dwx,dwz),dy=camera.screenY(dwx,dwz,y+scale*v[d+1]);
