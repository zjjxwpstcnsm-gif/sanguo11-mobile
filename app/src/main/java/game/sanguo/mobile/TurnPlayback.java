...
final class TurnPlayback {
    ...
    private void step(){
        ...
        if(work.fastForward()){
            ...
            if(work.done){work.clearPresentation();finished.run();return;}
        ...
        while(work.cursor<work.events.size()){
            ...
            event.applyVisual(work.visual);work.cursor++;work.fraction=0;work.criticalElapsed=0;current=null;applied=true;map.replayFrame(null,0);
        ...
        if(work.cursor>=work.events.size()){
            ...
            if(work.done&&!work.batchReady){finished.run();return;}
