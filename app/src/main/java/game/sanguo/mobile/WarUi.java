...
final class WarUi {
    ...
    void move(World.Unit u,Hex target){
        ...
        confirm("确认移动",route+"\n消耗移动力 "+plan.cost+"，移动后剩余 "+plan.remaining+"。\n仍可攻击、战法、计略、入城或待命。",()->apply.execute(w,()->w.orders.execute(plan)));
    }
