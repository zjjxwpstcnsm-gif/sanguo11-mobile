...
final class WorldUi {
    ...
    void districts(){List<Districts.District> list=w.districts.all();String[] names=new String[list.size()+2];names[0]="新设军团";for(int i=0;i<list.size();i++){Districts.District d=list.get(i);names[i+1]=d.name()+" · "+d.policy().label+" · "+d.cities().size()+"城 / "+(w.districts.sites(d).size()-d.cities().size())+"港关";}
        names[names.length-1]="批量托管后方城池";
    ...
    void city(World.City c){
        if(c.kind!=World.SiteKind.CITY){
            World.City parent=w.districts.parentCity(c.id);
            if(parent!=null&&parent.owner==w.player&&c.owner==w.player){
                new AlertDialog.Builder(a).setTitle(c.name+" · 随城托管").setMessage(w.districts.affiliation(c.id)+"。港关不能单独编入军团。")
                    .setPositiveButton("管理"+parent.name,(d,n)->city(parent)).setNegativeButton("返回",null).show();
            }else info(c.name+" · 随城托管",w.districts.affiliation(c.id));
            return;
        }
    ...
    private boolean rulerCity(World.City c){return w.districts.rulerCity(c.id);}
    private void rear(){
        Territory territory=new Territory(w);List<World.City> cities=new ArrayList<>();CampaignAi ai=new CampaignAi(w);
        for(World.City c:w.cities)if(c.kind==World.SiteKind.CITY&&c.owner==w.player&&w.districts.city(c.id)==null&&!rulerCity(c)&&!territory.frontline(c.id)&&ai.incoming(c)==0)cities.add(c);
        ...
        new AlertDialog.Builder(a).setTitle("后方托管 · 勾选城池").setMultiChoiceItems(cities.stream().map(c->c.name+" · 闲将"+w.idle(c).size()).toArray(String[]::new),checked,(d,n,yes)->checked[n]=yes)
    ...
    private String describe(Districts.District d){StringBuilder b=new StringBuilder(d.policy().label+"\n都督："+(d.leader()<0?"暂无在城武将":w.officer(d.leader()).name)+"\n军团行动力："+d.points()+" / 60\n");
        ...
        for(int id:w.districts.sites(d)){World.City c=w.city(id);b.append(c.name).append(" · 闲将").append(w.idle(c).size()).append(" · 金").append(c.gold).append(" · 粮").append(c.food).append('\n').append(new DistrictManagement(w).forecast(c)).append('\n');}
    ...
    void batch(List<World.City> visible){
        List<World.City> cities=new ArrayList<>();for(World.City c:visible)if(c.kind==World.SiteKind.CITY&&c.owner==w.player)cities.add(c);
        if(cities.isEmpty()){info("批量划入军团","当前筛选下没有己方据点");return;}
        boolean[] selected=new boolean[cities.size()];
        new AlertDialog.Builder(a).setTitle("批量划入军团 · 选择城池").setMultiChoiceItems(cities.stream().map(c->c.name+(rulerCity(c)?" · 君主驻地，不可托管":"")).toArray(String[]::new),selected,(d,i,yes)->selected[i]=yes).setPositiveButton("预览",(d,n)->{
    ...
    private void members(Districts.District old){
        List<World.City> list=new ArrayList<>();for(World.City c:w.cities){Districts.District other=w.districts.city(c.id);boolean ruler=false;for(World.Officer o:w.officers)if(o.owner==w.player&&o.role==Strategy.Role.RULER&&o.cityId==c.id)ruler=true;if(c.kind==World.SiteKind.CITY&&c.owner==w.player&&!rulerCity(c)&&(other==null||other==old))list.add(c);}
        ...
        new AlertDialog.Builder(a).setTitle("选择城池 · 附属港关自动跟随").setMultiChoiceItems(list.stream().map(c->c.name).toArray(String[]::new),selected,(d,n,checked)->selected[n]=checked).setPositiveButton("下一步",(d,n)->{
