...
/** Title tables follow SAN11; acquisition timing remains the mobile mutation-boundary rule.
 * Read-only labels never mutate a save or a preview. See docs/RULER_TITLES.md. */
public final class Governance {
    private static final int LEGACY_MAGIC=0x47563636,MAGIC=0x47563637;
    ...
    public boolean canNameNation(int side){return side>=0&&side<w.factions.length&&currentTitle(side)==RulerTitles.Title.EMPEROR;}
    public String titleProgress(int side){
        RulerTitles.Title current=currentTitle(side),next=current.next();int cities=cityCount(side);
        return "领有"+cities+"城（不计港口/关卡） · "+current.label+" · 君主基础指挥"+current.troops
            +(next==null?" · 已达最高爵位":"\n下一爵位："+next.label+"，需"+next.cities+"城，还差"+Math.max(0,next.cities-cities)+"城");
    }
    ...
    public World.Result nameNation(int side,String name){w.reports.prepare();
        ...
        if(!canNameNation(side))return w.fail("君主成为皇帝后方可设定国号");
    ...
    void write(DataOutputStream d)throws IOException{
        d.writeInt(MAGIC);d.writeInt(w.fieldUnits().size());
    ...
    void read(DataInputStream d)throws IOException{
        int magic=d.readInt();if(magic!=MAGIC&&magic!=LEGACY_MAGIC)throw new IOException("治理存档段错误");int n=d.readInt();
    ...
    void validate()throws IOException{
        ...
        for(Map.Entry<Integer,Integer> e:grades.entrySet())if(e.getKey()<0||e.getKey()>=w.factions.length||e.getValue()<0||e.getValue()>RulerTitles.Title.EMPEROR.grade())throw new IOException("君主爵位错误");
        Set<String> seen=new HashSet<>();for(Map.Entry<Integer,String> e:nations.entrySet())if(e.getKey()<0||e.getKey()>=w.factions.length||!canNameNation(e.getKey())||!validName(e.getValue())||!seen.add(e.getValue()))throw new IOException("国号无效");
    }
}
