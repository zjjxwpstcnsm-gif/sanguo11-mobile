...
public final class MapEditSession {
    ...
    public synchronized int paint(String label,Collection<Hex> hexes,World.Terrain terrain)throws IOException {
        ...
        if(before.isEmpty())return 0;Change change=new Change(label,before,after,null,null,128L+before.size()*96L);applyTerrain(after);record(change);return before.size();
    }
