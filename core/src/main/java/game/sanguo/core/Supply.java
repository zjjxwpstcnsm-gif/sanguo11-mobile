...
public final class Supply {
    ...
    public World.Result raid(int actor,int mission){
        ...
        w.domestic.missions.remove(m);List<String> captured=new ArrayList<>(),escaped=new ArrayList<>();
        for(int id:m.crew()){World.Officer courier=w.officer(id);
            ...
            if(caught){w.government.capture(courier,u);captured.add(courier.name);}else{w.retreat(courier,m.hex);escaped.add(courier.name);}
        }
        w.campaign.earn(u.owner,40);w.checkVictory();
        w.battleOutcome("运输队被击破；缴获 金+"+gold+"、粮+"+food+"，加入攻击部队；俘虏："+(captured.isEmpty()?"无":String.join("、",captured)+"（随军押送）")+"；逃脱："+(escaped.isEmpty()?"无":String.join("、",escaped))+"；其余货物损失");
