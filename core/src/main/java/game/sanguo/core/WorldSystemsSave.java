...
final class WorldSystemsSave {
    ...
    static void validate(World w)throws IOException{
        ...
        for(WorldEvents.Camp c:e.camps){
            ...
            // cost() includes events.at(h): a camp blocks movement on its own cell, not its saved validity.
            require(w.terrain[c.hex.q][c.hex.r]!=World.Terrain.MOUNTAIN&&w.terrain[c.hex.q][c.hex.r]!=World.Terrain.NON_NAVIGABLE_WATER&&!w.army.water(c.hex)&&w.cityAt(c.hex)==null&&w.unitAt(c.hex)==null&&w.domestic.at(c.hex)==null&&w.war.at(c.hex)==null,"贼寨地块冲突");
        }
