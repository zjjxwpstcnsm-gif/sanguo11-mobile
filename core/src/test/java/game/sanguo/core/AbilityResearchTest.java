...
public final class AbilityResearchTest {
    ...
    private static void lossAndLegacy()throws Exception{
        ...
        try(InputStream in=AbilityResearchTest.class.getResourceAsStream("/legacy-v9.sg11.b64")){World old=SaveCodec.decode(Base64.getMimeDecoder().decode(in.readAllBytes()));check(old.campaign.projects().size()==1&&old.abilities.training().isEmpty(),"real v9 running legacy project preserved");check(ByteBuffer.wrap(bytes(old),4,4).getInt()==22,"migrates v9 to v18");for(int i=0;i<3;i++)tick(old);check(old.officer(3001).aptitude[0]==2&&old.campaign.projects().isEmpty(),"legacy task finishes original reward once");}
