...
public final class ContentTest {
    ...
    public static void main(String[] args)throws Exception {
        long start=System.nanoTime();ContentCatalog c=ContentCatalog.get();long load=System.nanoTime()-start;
        ...
        for(int side=0;side<3;side++){
            ...
            check(w.nextTurn().ok&&w.turn==1,"new pack actual AI turn");byte[] save=SaveCodec.encode(w);World restored=SaveCodec.decode(save);check(Arrays.equals(save,SaveCodec.encode(restored)),"new pack exact v6 snapshot");
