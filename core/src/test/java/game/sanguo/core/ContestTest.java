    ...
    private static void saveAndCorruption()throws Exception{
        ...
        for(int version=1;version<=8;version++){
            try(InputStream in=ContestTest.class.getResourceAsStream("/legacy-v"+version+".sg11.b64")){
                if(in==null)continue;World w=SaveCodec.decode(Base64.getMimeDecoder().decode(in.readAllBytes()));check(!w.contests.busy()&&w.contests.profiles.isEmpty(),"old save gains no invented profiles");check(ByteBuffer.wrap(bytes(w),4,4).getInt()==17,"old save writes v17");
            }
        }
