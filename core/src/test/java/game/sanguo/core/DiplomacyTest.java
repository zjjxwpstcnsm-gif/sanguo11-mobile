...
public final class DiplomacyTest {
    ...
    static void migration()throws Exception{
        try(InputStream in=DiplomacyTest.class.getResourceAsStream("/save-v16-before-diplomacy.b64")){
            ...
            check(w.government.prisoner(7).unitId==1&&w.diplomacy.aids.isEmpty(),"old escort migrates without invented diplomacy");check(bytes(w)[7]==25&&Arrays.equals(bytes(w),bytes(copy(w))),"current save exact roundtrip");
        }
