...
public final class DomesticTest {
    ...
    private static void legacy()throws Exception{
        for(String file:new String[]{"/m0-v1.sg11.b64","/m1-v2.sg11.b64"}){
            ...
            World clone=copy(w);check(SaveCodec.encode(w)[7]==5,"new writes use save v5");for(int i=0;i<5&&!w.gameOver();i++){next(w);next(clone);check(Arrays.equals(SaveCodec.encode(w),SaveCodec.encode(clone)),"legacy deterministic continuation");}
        }
    }
