...
public final class LogisticsCampaignTest {
    ...
    public static void main(String[] args)throws Exception{migration();transport();starvationAndBlock();capacityAndRedirect();raidAndDeath();permissionsAndSupport();localProgress();longEconomy();campaign();System.out.println("PASS: "+checks+" v026 logistics/campaign assertions.");}
    ...
    private static void longEconomy()throws Exception{
        ...
        for(int t=0;t<36;t++){
            ok(w.nextTurn());ok(replay.nextTurn());check(Arrays.equals(bytes(w),bytes(replay)),"36-turn multi district save/reload deterministic at "+t);replay=copy(replay);
            ...
            check(w.officers.size()==23,"no invented staff");
