...
public final class ScenarioTest {
    ...
    private static void data()throws Exception {
        List<World> packs=ScenarioCatalog.all();check(packs.size()==5,"four original packs plus sourced officer drill");
    ...
    private static void migration()throws Exception {
        ...
        byte[] migrated=SaveCodec.encode(w);check(migrated[7]==10,"new writes use save v9");
