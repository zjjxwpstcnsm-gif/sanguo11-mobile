...
public final class ScenarioTest {
    ...
    private static void data()throws Exception {
        List<World> packs=TestScenarios.all();check(packs.size()==18,"existing packs and three playable mobile sandboxes");
        ...
        World second=TestScenarios.load("regional-sandbox",2);w.city(300).food=1;
        check(second.city(300).food==40000,"new games have independent mutable state");
        String text=resource("/test-scenarios/regional-sandbox.properties");
        ...
        bad(text.replaceFirst("terrain.0=.","terrain.0=?"),0,"unknown terrain");
        ...
        try{TestScenarios.load("../regional-sandbox",0);throw new AssertionError("path traversal accepted");}catch(IOException expected){checks++;}
        bad(text+"#".repeat(1024*1024),0,"oversized input");
        World snapshot=TestScenarios.load("regional-sandbox",1);snapshot.dataHash="xyz";invalidSave(snapshot,"bad data hash");
        snapshot=TestScenarios.load("regional-sandbox",1);snapshot.active=3;invalidSave(snapshot,"bad active faction");
        snapshot=TestScenarios.load("regional-sandbox",1);snapshot.winner=0;invalidSave(snapshot,"premature winner");
    }
    private static void migration()throws Exception {
        ...
        byte[] migrated=SaveCodec.encode(w);check(migrated[7]==25,"new writes use save v19");
    ...
    private static void factions()throws Exception {
        for(int player=0;player<3;player++) {
            World w=TestScenarios.load("regional-sandbox",player);World.City home=w.home();World.Officer o=w.idle(home).get(0);
        ...
        World w=TestScenarios.load("regional-sandbox",0);
        ...
        w=TestScenarios.load("regional-sandbox",0);w.cities.removeIf(c->c.owner==0);w.officers.removeIf(o->o.owner==0);dropMissingTalents(w);w.checkVictory();
    ...
    private static void campaigns()throws Exception {
        for(int player=0;player<3;player++) {
            World a=TestScenarios.load("regional-sandbox",player),b=TestScenarios.load("regional-sandbox",player);
