...
public final class StrategyTest {
    ...
    private static void governors()throws Exception{
        ...
        check(w.city(10).gold==gold+960&&w.city(10).food==food-720,"monthly gold uses governor bonus; grain harvest waits until season");
    ...
    private static void saving()throws Exception{
        ...
        byte[] bytes=SaveCodec.encode(w);check(ByteBuffer.wrap(bytes,4,4).getInt()==17,"writes save v17");World restored=SaveCodec.decode(bytes);
    ...
    private static void legacy()throws Exception{
        ...
        for(int v=1;v<=3;v++){
            ...
            byte[] upgraded=SaveCodec.encode(w);check(ByteBuffer.wrap(upgraded,4,4).getInt()==17,"legacy resaves as v17");World paired=SaveCodec.decode(upgraded);
