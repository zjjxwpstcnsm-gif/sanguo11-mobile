...
public final class StrategyTest {
    ...
    private static void saving()throws Exception{
        ...
        byte[] bytes=SaveCodec.encode(w);check(ByteBuffer.wrap(bytes,4,4).getInt()==18,"writes save v17");World restored=SaveCodec.decode(bytes);
    ...
    private static void legacy()throws Exception{
        ...
        for(int v=1;v<=3;v++){
            ...
            byte[] upgraded=SaveCodec.encode(w);check(ByteBuffer.wrap(upgraded,4,4).getInt()==18,"legacy resaves as v17");World paired=SaveCodec.decode(upgraded);
