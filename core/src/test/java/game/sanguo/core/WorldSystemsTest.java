...
public final class WorldSystemsTest {
    ...
    private static void branchMigration()throws Exception{
        for(String branch:new String[]{"world","march"})try(InputStream in=WorldSystemsTest.class.getResourceAsStream("/legacy-v13-"+branch+".sg11.b64")){
            ...
            check(bytes(w)[7]==15&&Arrays.equals(bytes(w),bytes(copy(w))),"both v13 layouts upgrade to stable v15");World b=copy(w);for(int i=0;i<3;i++){ok(w.nextTurn());ok(b.nextTurn());check(Arrays.equals(bytes(w),bytes(b)),"old branch resumes with all new systems");}
        }
    }
