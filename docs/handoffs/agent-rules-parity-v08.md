...
v1～v6 的原有布局不变。v7 在 ArmySave 后、日志前追加 RulesSave：标记0x52554C37；单位数及每个稳定单位ID/移动预算/已用移动；武将数及每个稳定武将ID/UTF skillId/sex ordinal。读取验证重复或不存在ID、数量、预算、ID格式。静态数据缺失不抹除未知技能；既有 scenarioId 保留。
...
现有回归保留。原断言的必要修正：存档头从6变7；普通移动不再结束行动；兵器攻击城市改为对应战法；测试模拟新旬使用完整预算重置。未删除失败测试。新 RulesParityTest 目前523断言，包括纯预览、取消/非法状态不变、预算/水陆转换、组合技能与防御优先、移动后攻击/入城、真实v6任务、确定性三势力长局与重载。

Android 构建/Lint/三种横屏安装操作、APK摘要与签名：等待本分支CI，完成后更新。未使用ARM真机，不能给出真机结论。签名未比较前不保证覆盖旧版安装，也不建议卸载有重要存档的旧版。
