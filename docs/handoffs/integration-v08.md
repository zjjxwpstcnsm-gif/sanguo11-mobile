...
本地合并代码已通过既有核心回归、2637资料断言、575规则断言与15 Python测试；另外通过143条资料到真实规则联合断言与51730条UI/相机断言。Android与最终APK、签名结果待本集成分支独立CI，不以两个独立PR的成功替代。无ARM真机，未做真机测试。
...
追加纳入 Agent 2 `08da7a68592bb4834c5a08ebf824dedc27bd0d35`。其实际截图/失败记录表明一次滚动后首行仍部分可见，原断言误以为一次滚动必定换首行；保留换首行与重建恢复断言，改为有限次真实滚动，并纳入首行像素偏移保存、紧凑行摘要和真实玩家/AI部队交战验证。联合版本重新执行完整Android流程。
