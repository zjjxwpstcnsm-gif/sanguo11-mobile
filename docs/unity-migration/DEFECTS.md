...
| U00-03 | BLOCKED | Current environment cannot download Gradle 8.13 and has no Android SDK / `javac` executable; full Android build cannot run. Existing main CI emulator flow already fails a scenario text assertion. Pure-Java UI suite passed with a temporary `java -m jdk.compiler/...` shim. |
