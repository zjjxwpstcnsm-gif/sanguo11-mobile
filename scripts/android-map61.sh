...
cd "$(dirname "$0")/.."
mkdir -p out/android
apk=$(find out -maxdepth 1 -name 'sanguo11-mobile-v0.61.0-*.apk' -print -quit)
test -n "$apk"
...
adb install -r out/upgrade-v060.apk > out/UPGRADE_BASE_INSTALL.txt
adb push out/legacy61/raw/baseline-v060-coalition-190.sg11 /data/local/tmp/legacy-v060.sg11
adb shell mkdir -p /data/data/game.sanguo.mobile.dev/files
adb shell cp /data/local/tmp/legacy-v060.sg11 /data/data/game.sanguo.mobile.dev/files/manual1.sg11
...
set +e
timeout 1100 adb shell am instrument -w -e reference61 true game.sanguo.mobile.dev.test/game.sanguo.mobile.GameSmokeRunner > out/reference61-instrumentation.txt
instrumentation_exit=$?
set -e
adb logcat -b all -d -v threadtime > out/reference61-logcat.txt
adb pull /sdcard/Android/data/game.sanguo.mobile.dev/files/smoke out/android/reference61
reference=false
if test "$instrumentation_exit" -eq 0 && grep -q 'REFERENCE61 ANDROID PASS:' out/reference61-instrumentation.txt && ! grep -q 'FATAL EXCEPTION' out/reference61-logcat.txt;then reference=true;fi
...
printf '{"reference61":%s,"retained_native200_gameplay":%s,"in_place_signed_upgrade":true,"old_manual_save_byte_identical_after_install":true,"api":29,"abi":"x86_64","physical_arm_tested":false}\n' "$reference" "$preserved" > out/ANDROID_STATUS.json
