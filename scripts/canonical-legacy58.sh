...
set -euo pipefail
cd "$(dirname "$0")/.."
base=29c18ec127b7baeafaf9faf9f3b03ee5c1cd2a78
...
cat > out/legacy-encoder/CanonicalLegacy58.java <<'JAVA'
import java.nio.file.*;
import game.sanguo.core.SaveCodec;
public final class CanonicalLegacy58 {
 public static void main(String[] a)throws Exception {
  for(String id:new String[]{"coalition-190","heroes-250"}) {
   String name="baseline-v057-"+id+".sg11";
   Files.write(Path.of(a[1],name),SaveCodec.encode(SaveCodec.decode(Files.readAllBytes(Path.of(a[0],name)))));
  }
 }
}
JAVA
javac --release 17 -cp out/legacy-encoder/classes -d out/legacy-encoder/classes out/legacy-encoder/CanonicalLegacy58.java
java -cp out/legacy-encoder/classes:out/legacy-encoder/src/core/src/main/resources CanonicalLegacy58 out/baseline/android out/baseline-canonical
printf '%s\n' "$base" > out/LEGACY_ENCODER_COMMIT
sha256sum out/baseline/android/baseline-v057-*.sg11 out/baseline-canonical/* > out/LEGACY_SAVE_HASHES
