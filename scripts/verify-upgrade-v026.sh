#!/usr/bin/env bash
set -euo pipefail
mkdir -p app/build/smoke/upgrade-v026 app/build/upgrade
# Start a separate genuine v0.26 installation after the v0.9 gate.
adb uninstall game.sanguo.mobile.dev
adb install app/build/upgrade/v026.apk
...
cmp app/build/upgrade/v20.sg11 app/build/upgrade/v20-old-app.sg11
...
adb logcat -c
adb shell am instrument -w -e upgrade 26 game.sanguo.mobile.dev.test/game.sanguo.mobile.GameSmokeRunner | tee app/build/smoke/upgrade-v026/instrumentation.txt
adb logcat -d > app/build/smoke/upgrade-v026/logcat.txt
adb pull /sdcard/Android/data/game.sanguo.mobile.dev/files/smoke app/build/smoke/upgrade-v026/screenshots
python3 - <<'PY'
from pathlib import Path
root=Path('app/build/smoke/upgrade-v026')
assert 'UPGRADE26 PASS:' in (root/'instrumentation.txt').read_text()
