...
def inventory(root):
 ...
 for family in ('sites','field'):
  ...
  for p in sorted((root/f'app/src/main/assets/3d/{family}').glob('*.glb')):
   atlas=root/f'app/src/main/assets/3d/{family}'/('unit-atlas.png' if p.stem.startswith('unit-') else 'atlas.png')
...
if __name__=='__main__':
 ...
 with tempfile.TemporaryDirectory(prefix='sanguo-assets-') as temp:
  ...
  catalog={'schema':1,'cohort':'R10-v5','generatorDependencies':{'python':'3.11+','Pillow':'12.3.0','numpy':'2.3.5','etcpak':'0.9.15','texture2ddecoder':'1.0.6','Filament':'1.56.0 / matc56'},'sourceHashes':{s:digest(ROOT/s) for s in ['tools/3d/build_sites.py','tools/3d/build_field_assets.py','tools/3d/compress_atlases.py']},'assets':inventory(out)}
