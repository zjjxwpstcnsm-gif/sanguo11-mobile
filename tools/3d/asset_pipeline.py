...
def inventory(root):
 ...
 for family in ('sites','field'):
  ...
  for p in sorted((root/f'app/src/main/assets/3d/{family}').glob('*.glb')):
   ...
   result.append(dict(id=f'{family}/{p.stem}',version=2,runtime=str(p.relative_to(root)),sha256=digest(p),bytes=len(b),triangles=idx['count']//3,primitives=1,lod=int(p.stem[-1]) if '-lod' in p.stem else None,bounds=a['min']+a['max'],pivot=[0,0,0],axes='+Y up / +Z forward',unit='one projected tile span',license='CC0-1.0',author='project procedural asset source',source='tools/3d/build_sites.py' if family=='sites' else 'tools/3d/build_field_assets.py',texture=f'3d/{family}/atlas.png',textureSha256=digest(atlas),colorSpace='sRGB',animation='rigid parts v1; no skin/root motion' if p.stem.startswith('unit-') else 'static',mapping='SiteVisual.model' if family=='sites' else 'Vegetation.build' if p.stem.startswith('tree') else 'FieldAssets.unit/facility; construction/fire state'))
 return result
if __name__=='__main__':
 ...
 with tempfile.TemporaryDirectory(prefix='sanguo-assets-') as temp:
  ...
  catalog={'schema':1,'cohort':'R06-v2','generatorDependencies':{'python':'3.11+','Pillow':'12.3.0','numpy':'2.3.5','etcpak':'0.9.15','texture2ddecoder':'1.0.6','Filament':'1.56.0 / matc56'},'sourceHashes':{s:digest(ROOT/s) for s in ['tools/3d/build_sites.py','tools/3d/build_field_assets.py','tools/3d/compress_atlases.py']},'assets':inventory(out)}
